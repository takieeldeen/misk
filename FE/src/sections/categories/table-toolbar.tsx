...
export function TableToolbar({ filters, options }: Props) {
  const popover = usePopover();
  const { t, i18n } = useTranslate();
  const mdUp = useResponsive('up', 'md');
  const local = useSetState<ICategoryTableFilters>({
    ...
    <>
      ...
      <FormControl sx={{ flexShrink: 0, width: { xs: 1, md: 300 } }}>
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField {...params} label={t('common.status')} placeholder={t('common.status')} />
          )}
